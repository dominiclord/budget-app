...
trait DescribablePropertyTrait
{
     ...
     *
     * @param array|null $filters  Array of properties to filter out.
...
// Hack!
        ...
        foreach ($props as $propertyIdent) {
            ...
            if ($property->isFiltered($propertyIdent, $filters)) {
                continue;
            }
    ...
    {
        if (!is_string($propertyIdent) || $filters === null) {
            return false;
        }

        foreach ($filters as $filterIdent => $filterData) {
            if ($propertyIdent === $filterData) {
                return true;
            }
            unset($filterIdent);
            unset($filterData);
